...
async def get_users(event):
    ...
    if not sender.id == me.id:
        remix = await event.reply("`Sedang Memproses...`")
    else:
        remix = await event.edit("`Memproses...`")
    remixubot = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await remix.edit("`Maaf, Tidak dapat menambahkan pengguna di sini`")
    s = 0
    f = 0
    error = 'None'

    await remix.edit("**TerminalStatus**\n\n`Mengumpulkan Pengguna...`")
    async for user in event.client.iter_participants(remixubot.full_chat.id):
        try:
            if error.startswith("Too"):
                return await remix.edit(f"**Terminal Kesalahan...**\n(`Mungkin Mendapat Kesalahan Batas dari telethon Tolong Coba Lagi`)\n**Error** : \n`{error}`\n\n• Menambahkan Pengguna `{s}`\n• Gagal Menambahkan Pengguna `{f}`")
            await event.client(functions.channels.InviteToChannelRequest(channel=chat, users=[user.id]))
            s = s + 1
            await remix.edit(f"**Terminal Berjalan...**\n\n• Menambahkan Pengguna `{s}`\n• Gagal Menambahkan Pengguna `{f}`\n\n**Kesalahan Terakhir :** `{error}`")
        except Exception as e:
            error = str(e)
            f = f + 1
    return await remix.edit(f"**Terminal Selesai** \n\n• Sukses Menambahkan Pengguna `{s}`\n• Gagal Menambahkan Pengguna `{f}`")
